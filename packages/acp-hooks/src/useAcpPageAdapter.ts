 ...
 *    platform chat areas and tool-call panels expect.
...
import { mergeToolCall } from "./domain";

/** Minimal session summary produced for sidebar rendering. */
...
export interface UseAcpPageAdapterOptions {
  /** The gateway to connect to. Pass `null` when none is configured. */
  gateway: Gateway | null;
  /** Platform-specific ACP client state (from the platform's `useAcpClient`). */
  acp: AcpClientState;
  /** Platform-specific storage adapter (localStorage, MMKV, etc.). */
  storage: StorageAdapter;
  /** Auto-archive threshold string, e.g. "3d". Empty string disables it. */
  autoArchiveThreshold: string;
  /** Optional callback to discover the agent's cwd before session operations. */
  getAgentCwd?: GetAgentCwd;
  /** Optional confirmation before archiving. Return false to cancel. */
 ...
 *
 * @param options Adapter options including the gateway, transport factory,
 * storage adapter, and settings.
 */
export function useAcpPageAdapter(
  options: UseAcpPageAdapterOptions,
  ...
  // default to "/").
  useEffect(() => {
    if (!gateway || !getAgentCwd) return;
    let cancelled = false;

    (async () => {
      try {
        const cwd = await getAgentCwd(gateway);
        if (!cancelled && cwd) {
  ...
  }, [gateway?.id, gateway?.url, getAgentCwd]);
  ...
  // Sessions archived on the client side. The archive list is persisted via
  // the provided storage adapter (keyed by gateway id) so archived sessions
  // stay hidden after a restart. `session/list` results are filtered against
  // this set before being rendered.
  const gatewayId = gateway?.id ?? null;
  const archivedSessions = useArchivedSessions({ gatewayId, storage });
  ...
  // Auto-archive threshold from settings. `""` disables it.
  ...
  // open list is persisted (keyed by gateway id) so a restart keeps track of
  // which sessions this client holds open. It exempts sessions from automatic
  // archiving and drives the close-before-archive flow.
  const openSessions = useOpenSessions({ gatewayId, storage });
  ...
  const onArchiveSession = useCallback(
    async (id: string) => {
      if (!id) return;
      if (onConfirmArchive) {
        const confirmed = await onConfirmArchive();
        if (!confirmed) return;
      }
  ...
  const onDeleteSession = useCallback(
    async (id: string) => {
      ...
      if (supportsDelete && onConfirmDelete) {
